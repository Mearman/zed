...
impl AcpThreadView {
        ...
        match &self.thread_state {
            ...
            ThreadState::LoadError(error) => match error {
                LoadError::NotInstalled { .. } => {
                    format!("{} Not Installed", self.agent.name()).into()
                }
    ...
    fn render_load_error(
        ...
        let (title, message, action_slot): (_, SharedString, _) = match e {
            LoadError::NotInstalled => return self.render_not_installed(None, window, cx),
            LoadError::Unsupported {
                command: path,
                current_version,
                minimum_version,
            } => {
                return self.render_not_installed(
                    Some((path, current_version, minimum_version.as_ref())),
                    window,
                    cx,
                );
            }
    ...
    fn render_not_installed(
        &self,
        existing_version: Option<(&SharedString, &SharedString, Option<&SharedString>)>,
        _window: &mut Window,
        cx: &mut Context<Self>,
    ) -> AnyElement {
        let (heading_label, description_label) =
            if let Some((path, version, minimum_version)) = existing_version {
                (
