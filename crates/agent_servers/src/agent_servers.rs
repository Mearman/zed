...
impl AgentServerDelegate {
    ...
    fn get_or_npm_install(
        ...
        let Some(node_runtime) = self.project.read(cx).node_runtime().cloned() else {
            return Task::ready(Err(anyhow!("Missing node runtime")));
        };

        cx.spawn(async move |cx| {
            if let Some(settings) = settings && !settings.ignore_system_version {
                let bin = find_bin_in_path(binary_name, &self.project, cx).await.ok_or(LoadError::NotInstalled)?;
            ...
            cx.background_spawn(async move {
                ...
                self.status_tx.send("Checking for latest version…".into())?;
                ...
                if should_install {
                    self.status_tx.send("Installing latest version…".into())?;
                    node_runtime
